...
class DrawingUtil {
    ...
    static drawTRectBeaker(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (var j = 0; j < 2; j++) {
            ...
            DrawingUtil.drawLine(context, -size / 2, 0, -size / 2, -size * sf2 * (1 - 2 * j))
            context.restore()
        }
        context.fillRect(-size / 2, -size * sf4, size, size * sf4)
        context.restore()
    }
...
class TRBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new TRBNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawTRBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : TRBNode {
        var curr : TRBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
